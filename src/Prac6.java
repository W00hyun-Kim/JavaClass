...
public class Prac6 {
	public static void main(String[] args) throws IOException {
		//지정한 경로의 파일 객체 생성하기
		File k6_f = new File("D:\\test\\전국주차장정보표준데이터.txt");
		...
		while ((k6_readtxt = k6_br.readLine()) != null) {
	         ...
	         //만약 경도 위도가 따로 명시되어있지않으면 pass~
	         if(k6_field[28]=="" || k6_field[29]=="") {
	        	 continue;
	         } else {	        	 
	        	 double k6_dist = Math.sqrt(Math.pow(Double.parseDouble(k6_field[28]) - k6_lat, 2)
	        			 + Math.pow(Double.parseDouble(k6_field[29]) - k6_lng, 2));	   			         
	        	 //최소거리 구하기
	        	 if(k6_dist<k6_min) {
	        		 k6_min = k6_dist;
	        		 k6_nearest = k6_field[1];
	        	 }	
	        	 //최대거리 구하기
	        	 if(k6_dist>k6_max) {
	        		 k6_max = k6_dist;
